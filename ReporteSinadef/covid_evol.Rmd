...
url.sinadef <- "https://cloud.minsa.gob.pe/s/NctBnHXDnocgWAg/download?path=%2F&files=SINADEF_DATOS_ABIERTOS_20042021.xlsx"
...
ggplot(ds, aes(fecha, pais)) +
  ...
  labs(x = "Fecha", y = "Número de fallecidos diarios",
       title = paste("Fallecidos diarios a nivel nacional hasta el", ultimo.dia, "de", ultimo.mes.texto),
       subtitle = paste("Exceso con respecto a la línea roja:", round(sum(ds$dif)))) +
  # annotate(geom = "text", x = as.Date("2020-06-20", "%Y-%m-%d"), y = 1140,
  #          label = "No votes por golpistas", size = 5, color = "darkblue", hjust = 0, vjust = 1) +
  # annotate(geom = "text", x = as.Date("2021-01-25", "%Y-%m-%d"), y = 700,
  #          label = "Las vacunas sí sirven", size = 5, color = "darkblue", hjust = 0, vjust = 1) +
  ylim(0, NA) +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = 'cr', k = 18)) +
...
ggplot(ds, aes(fecha, pais)) +
  ...
  ylim(0, NA) +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = 'cr', k = 14)) +
...
ggplot(ds, aes(fecha, pais)) +
  ...
  ylim(0, NA) +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = 'cr', k = 15)) +
...
ggplot(ds, aes(fecha, pais)) +
  ...
  labs(x = "Fecha", y = "Número de fallecidos diarios",
       ...
       subtitle = paste("Exceso con respecto a la línea roja:", round(sum(ds$dif)))) +
  ylim(0, NA) +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = 'cr', k = 16)) +
...
## 4. Evolución del número de fallecidos diarios por provincia (más de 150000 habitantes)
...
```{r distrito, eval = FALSE}
## 5. Evolución del número de fallecidos diarios por distrito (más de 100000 habitantes)
