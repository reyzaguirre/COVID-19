...
url.sinadef <- "https://cloud.minsa.gob.pe/s/NctBnHXDnocgWAg/download?path=%2F&files=SINADEF_DATOS_ABIERTOS_04102020.xlsx"
...
ultimo.dia <- as.numeric(substr(ultimo.fecha - 1, 9, 10))
ultimo.mes <- as.numeric(substr(ultimo.fecha - 1, 6, 7))
...
temp <- nv.20[nv.20$provincia == "HUANCAVELICA", ]
...
ggplot(ds, aes(fecha, pais)) +
  geom_point() +
  labs(x = "Fecha", y = "Número de fallecidos diarios",
       title = "Fallecidos diarios en la provincia de Huancavelica por causa no violenta") + 
  ylim(0, NA) +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = 'cr'))

## Abancay

temp <- nv.20[nv.20$provincia == "ABANCAY", ]

ds <- docomp("count", "pais", "fecha", dfr = temp, method = 'slow')

texto <- paste("Exceso en número de fallecidos\npor 1000 habitantes:\n- Actual:",
               format(ex[ex$prov2 == "ABANCAY", "dif"] / 110.520, digits = 4))

ggplot(ds, aes(fecha, pais)) +
  geom_point() +
  labs(x = "Fecha", y = "Número de fallecidos diarios",
       title = "Fallecidos diarios en la provincia de Abancay por causa no violenta") + 
  ylim(0, NA) +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = 'cr')) +
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1)
