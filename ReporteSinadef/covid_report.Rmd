...
url.sinadef <- "https://cloud.minsa.gob.pe/s/NctBnHXDnocgWAg/download?path=%2F&files=SINADEF_DATOS_ABIERTOS_26062020.xlsx"
...
ggplot(ds, aes(fecha, pais)) +
  ...
  ylim(0, NA) +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = 'cr'))
#   xlim(as.Date.numeric(0, "2020-01-01"), as.Date.numeric(dim(ds)[1] + 33, "2020-01-01")) +
#   geom_smooth(data = ds[ds$fecha < "2020-06-23", ], method = 'gam', formula = y ~ s(x, bs = 'cr'), fullrange = TRUE, color = 2) + # rojo
#   geom_smooth(data = ds[ds$fecha < "2020-06-26", ], method = 'gam', formula = y ~ s(x, bs = 'cr'), fullrange = TRUE, color = 3) # azul
