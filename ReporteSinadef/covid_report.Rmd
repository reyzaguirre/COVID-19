...
url.sinadef <- "https://cloud.minsa.gob.pe/s/NctBnHXDnocgWAg/download?path=%2F&files=SINADEF_DATOS_ABIERTOS_01012021.xlsx"
...
ggplot(ds, aes(fecha, pais)) +
  ...
  geom_vline(xintercept = as.Date("2020-12-30", "%Y-%m-%d"), linetype = 2, color = 3, size = 1.5)
...
ggplot(ds, aes(fecha, pais)) +
  ...
  geom_vline(xintercept = as.Date("2020-12-13", "%Y-%m-%d"), linetype = 2, color = 7, size = 1.5)
...
ggplot(ds, aes(fecha, pais)) +
  ...
  geom_vline(xintercept = as.Date("2020-12-30", "%Y-%m-%d"), linetype = 2, color = 7, size = 1.5)
...
ggplot(dc, aes(grupo, ratio, colour = sexo)) +
  ...
  labs(x = "Edad (en años)",
       ...
       name = "Sexo",
       title = "Exceso de muerte para el 2020 con respecto a la media 2018-2019") +
