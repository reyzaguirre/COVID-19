...
url.sinadef <- "https://cloud.minsa.gob.pe/s/NctBnHXDnocgWAg/download?path=%2F&files=SINADEF_DATOS_ABIERTOS_23062020.xlsx"
...
ggplot(ds, aes(fecha, pais)) +
  ...
  ylim(0, NA) +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = 'cr'))
#   xlim(as.Date.numeric(0, "2020-01-01"), as.Date.numeric(dim(ds)[1] + 36, "2020-01-01")) +
#   geom_smooth(data = ds[ds$fecha < "2020-06-21", ], method = 'gam', formula = y ~ s(x, bs = 'cr'), fullrange = TRUE, color = 4) + # verde
#   geom_smooth(data = ds[ds$fecha < "2020-06-19", ], method = 'gam', formula = y ~ s(x, bs = 'cr'), fullrange = TRUE, color = 2) + # rojo
#   geom_smooth(data = ds[ds$fecha < "2020-06-17", ], method = 'gam', formula = y ~ s(x, bs = 'cr'), fullrange = TRUE, color = 3) # azul
...
temp$grupo <- "Superior"
temp$grupo[temp$educa %in% c("INICIAL / PRE-ESCOLAR", "NINGUN NIVEL / ILETRADO")] <- "Ninguno"
...
niveles <- c("Secundaria", "Superior", "Primaria", "Ninguno")
dc$Instrucción <- factor(dc$grupo, levels = niveles, ordered = TRUE)

ggplot(dc, aes(fecha, ratio, colour = Instrucción)) +
  geom_point() +
  labs(x = "Fecha",
       y = "Cociente entre el año 2020 y la media 2018-2019",
       title = "Número de fallecidos diarios a nivel nacional por causa no violenta
...
temp <- temp[!(temp$educa == "IGNORADO" | temp$educa == "SIN REGISTRO"), ]
...
niveles <- c("Secundaria", "Primaria", "Superior", "Ninguno")
dc$Instrucción <- factor(dc$grupo, levels = niveles, ordered = TRUE)
...
ggplot(dc, aes(fecha, ratio, colour = Instrucción)) +
  ...
  labs(x = "Fecha",
       y = "Cociente entre el año 2020 y la media 2018-2019",
       title = "Número de fallecidos diarios en Lima y Callao por causa no violenta
según nivel de instrucción") + 
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = 'cr'))
