...
url.sinadef <- "https://cloud.minsa.gob.pe/s/NctBnHXDnocgWAg/download?path=%2F&files=SINADEF_DATOS_ABIERTOS_18072020.xlsx"
...
# Calcular exceso acumulado 2020 con respecto al 2019-2018
...
ex$media1819 <- (ex$total19 + ex$total18) / 2

# Diferencia

ex$dif <- ex$total20 - ex$media1819

```

## 1. Introducción

Los datos del sistema Informático Nacional de Defunciones, SINADEF, pueden descargarse desde [este enlace](http://www.minsa.gob.pe/reunis/data/defunciones_registradas.asp) y cuentan con el registro de defunciones desde el año 2017. En la circunstancia actual, estos datos permiten evaluar el efecto de la epidemia de COVID-19 en el país. Es importante tener en cuenta que podría haber un efecto de cobertura (o falta de cobertura). En los años 2017 y 2018 el número de fallecidos es un `r paste0(format(dif17.19, digits = 3), "%")` y `r paste0(format(dif18.19, digits = 3), "%")` menor que en el 2019 respectivamente por lo que solo se utilizarán como referencia los datos del 2018 y el 2019. Sobre el factor cobertura, si se considera una tasa de mortalidad para el país de un 5.6 por mil, el número de fallecidos registrados en Sinadef para los años 2018 y 2019 estaría entre un 62% y 64%.
...
Los siguientes gráficos permiten evaluar la evolución de la epidemia a través de su efecto en el número de fallecimientos diarios por causa no violenta, a nivel nacional y en las provincias con mayor población. En algunos de los gráficos se incluye el exceso de fallecidos este año con respecto a la media de los años 2018 y 2019 expresado en número de fallecidos por mil habitantes y una marca para la fecha en la que se llegó a uno, dos y tres fallecidos en exceso por cada mil. Para la población total en todos los casos se toma como referencia la población registrada en el [censo del 2017](https://es.wikipedia.org/wiki/Anexo:Provincias_del_Perú).
...
texto <- paste("Exceso en número de fallecidos\npor 1000 habitantes:\n- Línea azul: 1\n- Actual:",
               format((total.nv[4] - (total.nv[3] + total.nv[2]) / 2) / 31237.385, digits = 4))
...
#   xlim(as.Date.numeric(0, "2020-01-01"), as.Date.numeric(dim(ds)[1] + 24, "2020-01-01")) +
...
ggplot(ds, aes(fecha, pais)) +
  ...
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1)  +
  geom_vline(xintercept = as.Date("2020-05-12", "%Y-%m-%d"), linetype = 2, color = 4, size = 1.5) +
  geom_vline(xintercept = as.Date("2020-06-08", "%Y-%m-%d"), linetype = 2, color = 3, size = 1.5) +
  geom_vline(xintercept = as.Date("2020-07-10", "%Y-%m-%d"), linetype = 2, color = 7, size = 1.5)
...
ggplot(ds, aes(fecha, pais)) +
  ...
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1)  +
  geom_vline(xintercept = as.Date("2020-06-30", "%Y-%m-%d"), linetype = 2, color = 4, size = 1.5)
...
ggplot(ds, aes(fecha, pais)) +
  ...
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1) +
  geom_vline(xintercept = as.Date("2020-06-14", "%Y-%m-%d"), linetype = 2, color = 4, size = 1.5) +
  geom_vline(xintercept = as.Date("2020-07-03", "%Y-%m-%d"), linetype = 2, color = 3, size = 1.5)
...
texto <- paste("Exceso en número de fallecidos\npor 1000 habitantes:\n- Línea azul: 1\n- Actual:",
               format(ex[ex$prov2 == "PIURA", "dif"] / 799.321, digits = 4))
...
ggplot(ds, aes(fecha, pais)) +
  ...
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1)  +
  geom_vline(xintercept = as.Date("2020-06-02", "%Y-%m-%d"), linetype = 2, color = 4, size = 1.5)
...
ggplot(ds, aes(fecha, pais)) +
  ...
  geom_vline(xintercept = as.Date("2020-05-31", "%Y-%m-%d"), linetype = 2, color = 3, size = 1.5)
...
ggplot(ds, aes(fecha, pais)) +
  ...
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1) +
  geom_vline(xintercept = as.Date("2020-05-18", "%Y-%m-%d"), linetype = 2, color = 4, size = 1.5) +
  geom_vline(xintercept = as.Date("2020-06-12", "%Y-%m-%d"), linetype = 2, color = 3, size = 1.5) +
  geom_vline(xintercept = as.Date("2020-07-16", "%Y-%m-%d"), linetype = 2, color = 7, size = 1.5)
...
ggplot(ds, aes(fecha, pais)) +
  ...
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1) +
  geom_vline(xintercept = as.Date("2020-06-25", "%Y-%m-%d"), linetype = 2, color = 4, size = 1.5)
...
ggplot(ds, aes(fecha, pais)) +
  ...
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1) +
  geom_vline(xintercept = as.Date("2020-05-02", "%Y-%m-%d"), linetype = 2, color = 4, size = 1.5) +
  geom_vline(xintercept = as.Date("2020-05-17", "%Y-%m-%d"), linetype = 2, color = 3, size = 1.5)
...
texto <- paste("Exceso en número de fallecidos\npor 1000 habitantes:\n- Línea azul: 1\n- Actual:",
               format(ex[ex$prov2 == "SULLANA", "dif"] / 311.454, digits = 4))
...
texto <- paste("Exceso en número de fallecidos\npor 1000 habitantes:\n- Actual:",
               format(ex[ex$prov2 == "HUAMANGA", "dif"] / 282.194, digits = 4))
...
texto <- paste("Exceso en número de fallecidos\npor 1000 habitantes:\n- Actual:",
               format(ex[ex$prov2 == "CAÑETE", "dif"] / 240.013, digits = 4))
...
texto <- paste("Exceso en número de fallecidos\npor 1000 habitantes:\n- Línea azul: 1\n- Actual:",
               format(ex[ex$prov2 == "HUAURA", "dif"] / 227.685, digits = 4))
...
ggplot(ds, aes(fecha, pais)) +
  ...
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1) +
  geom_vline(xintercept = as.Date("2020-07-09", "%Y-%m-%d"), linetype = 2, color = 4, size = 1.5)
...
ggplot(ds, aes(fecha, pais)) +
  ...
  geom_vline(xintercept = as.Date("2020-06-21", "%Y-%m-%d"), linetype = 2, color = 7, size = 1.5) +
  geom_vline(xintercept = as.Date("2020-07-17", "%Y-%m-%d"), linetype = 2, color = 2, size = 1.5)
...
temp <- nv.20[nv.20$provincia == "SATIPO", ]

ds <- docomp("count", "pais", "fecha", dfr = temp, method = 'slow')

texto <- paste("Exceso en número de fallecidos\npor 1000 habitantes:\n- Actual:",
               format(ex[ex$prov2 == "SATIPO", "dif"] / 203.985, digits = 4))
...
ggplot(ds, aes(fecha, pais)) +
  ...
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1)  +
  geom_vline(xintercept = as.Date("2020-07-07", "%Y-%m-%d"), linetype = 2, color = 4, size = 1.5)

## Jaen

temp <- nv.20[nv.20$provincia == "JAEN", ]

ds <- docomp("count", "pais", "fecha", dfr = temp, method = 'slow')

texto <- paste("Exceso en número de fallecidos\npor 1000 habitantes:\n- Actual:",
               format(ex[ex$prov2 == "JAEN", "dif"] / 185.432, digits = 4))
...
ggplot(ds, aes(fecha, pais)) +
  ...
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1)  +
  geom_vline(xintercept = as.Date("2020-06-23", "%Y-%m-%d"), linetype = 2, color = 4, size = 1.5)
...
ggplot(ds, aes(fecha, pais)) +
  ...
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1) +
  geom_vline(xintercept = as.Date("2020-07-10", "%Y-%m-%d"), linetype = 2, color = 4, size = 1.5)
...
ggplot(ds, aes(fecha, pais)) +
  ...
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1) +
  geom_vline(xintercept = as.Date("2020-05-23", "%Y-%m-%d"), linetype = 2, color = 4, size = 1.5) +
  geom_vline(xintercept = as.Date("2020-06-10", "%Y-%m-%d"), linetype = 2, color = 3, size = 1.5) +
  geom_vline(xintercept = as.Date("2020-07-18", "%Y-%m-%d"), linetype = 2, color = 7, size = 1.5)
...
temp <- nv.20[nv.20$provincia == "LA CONVENCION", ]

ds <- docomp("count", "pais", "fecha", dfr = temp, method = 'slow')

texto <- paste("Exceso en número de fallecidos\npor 1000 habitantes:\n- Actual:",
               format(ex[ex$prov2 == "LA CONVENCION", "dif"] / 147.148, digits = 4))

ggplot(ds, aes(fecha, pais)) +
  geom_point() +
  labs(x = "Fecha", y = "Número de fallecidos diarios",
       title = "Fallecidos diarios en la provincia de La Convención por causa no violenta") + 
  ylim(0, NA) +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = 'cr')) +
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1)
...
temp <- nv.20[nv.20$provincia == "SANCHEZ CARRION", ]

ds <- docomp("count", "pais", "fecha", dfr = temp, method = 'slow')

texto <- paste("Exceso en número de fallecidos\npor 1000 habitantes:\n- Actual:",
               format(ex[ex$prov2 == "SANCHEZ CARRION", "dif"] / 144.405, digits = 4))

ggplot(ds, aes(fecha, pais)) +
  geom_point() +
  labs(x = "Fecha", y = "Número de fallecidos diarios",
       title = "Fallecidos diarios en la provincia de Sánchez Carrión por causa no violenta") + 
  ylim(0, NA) +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = 'cr')) +
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1)
...
temp <- nv.20[nv.20$provincia == "BARRANCA", ]

ds <- docomp("count", "pais", "fecha", dfr = temp, method = 'slow')

texto <- paste("Exceso en número de fallecidos\npor 1000 habitantes:\n- Actual:",
               format(ex[ex$prov2 == "BARRANCA", "dif"] / 144.381, digits = 4))

ggplot(ds, aes(fecha, pais)) +
  geom_point() +
  labs(x = "Fecha", y = "Número de fallecidos diarios",
       title = "Fallecidos diarios en la provincia de Barranca por causa no violenta") + 
  ylim(0, NA) +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = 'cr')) +
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1)
...
temp <- nv.20[nv.20$provincia == "TALARA", ]

ds <- docomp("count", "pais", "fecha", dfr = temp, method = 'slow')

texto <- paste("Exceso en número de fallecidos\npor 1000 habitantes:\n- Actual:",
               format(ex[ex$prov2 == "TALARA", "dif"] / 144.150, digits = 4))

ggplot(ds, aes(fecha, pais)) +
  geom_point() +
  labs(x = "Fecha", y = "Número de fallecidos diarios",
       title = "Fallecidos diarios en la provincia de Talara por causa no violenta") + 
  ylim(0, NA) +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = 'cr')) +
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1)
...
temp <- nv.20[nv.20$provincia == "CHOTA", ]

ds <- docomp("count", "pais", "fecha", dfr = temp, method = 'slow')

texto <- paste("Exceso en número de fallecidos\npor 1000 habitantes:\n- Actual:",
               format(ex[ex$prov2 == "CHOTA", "dif"] / 142.984, digits = 4))

ggplot(ds, aes(fecha, pais)) +
  geom_point() +
  labs(x = "Fecha", y = "Número de fallecidos diarios",
       title = "Fallecidos diarios en la provincia de Chota por causa no violenta") + 
  ylim(0, NA) +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = 'cr')) +
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1)
...
temp <- nv.20[nv.20$provincia == "ANDAHUAYLAS", ]

ds <- docomp("count", "pais", "fecha", dfr = temp, method = 'slow')

texto <- paste("Exceso en número de fallecidos\npor 1000 habitantes:\n- Actual:",
               format(ex[ex$prov2 == "ANDAHUAYLAS", "dif"] / 142.477, digits = 4))

ggplot(ds, aes(fecha, pais)) +
  geom_point() +
  labs(x = "Fecha", y = "Número de fallecidos diarios",
       title = "Fallecidos diarios en la provincia de Andahuaylas por causa no violenta") + 
  ylim(0, NA) +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = 'cr')) +
  annotate(geom = "text", x = as.Date("2020-01-01", "%Y-%m-%d"), y = max(ds$pais),
           label = texto, size = 4.2, color = "darkblue", hjust = 0, vjust = 1)
...
prov <- c("Lima y Callao", "Arequipa", "Trujillo", "Piura", "Huancayo", "Maynas",
          ...
          "Morropón", "Tumbes", "Chanchamayo", "Pisco", "Sánchez Carrión",
          "Barranca", "Talara", "Chota", "Andahuaylas")
exce <- c(ex[ex$prov2 == "LIMA Y CALLAO", "dif"] / 9569468 * 1000,
          ...
          ex[ex$prov2 == "PIURA", "dif"] / 799321 * 1000,
          ...
          ex[ex$prov2 == "SULLANA", "dif"] / 311454 * 1000,
          ex[ex$prov2 == "SAN ROMAN", "dif"] / 307417 * 1000,
          ex[ex$prov2 == "TACNA", "dif"] / 306363 * 1000,
          ex[ex$prov2 == "HUANUCO", "dif"] / 293397 * 1000,
          ex[ex$prov2 == "HUAMANGA", "dif"] / 282194 * 1000,
          ex[ex$prov2 == "CAÑETE", "dif"] / 240013 * 1000,
          ex[ex$prov2 == "HUAURA", "dif"] / 227685 * 1000,
          ...
          ex[ex$prov2 == "JAEN", "dif"] / 185432 * 1000,
          ...
          ex[ex$prov2 == "SANCHEZ CARRION", "dif"] / 144405 * 1000,
          ex[ex$prov2 == "BARRANCA", "dif"] / 144381 * 1000,
          ex[ex$prov2 == "TALARA", "dif"] / 144150 * 1000,
          ex[ex$prov2 == "CHOTA", "dif"] / 142984 * 1000,
          ex[ex$prov2 == "ANDAHUAYLAS", "dif"] / 142477 * 1000)
alti <- c(161, 2335, 34, 36, 3246, 105, 3399, 5, 406, 154, 2750, 65, 3824,
          562, 1898, 2761, 40, 30, 97, 3810, 631, 350, 729, 188, 3052, 92,
          6, 751, 17, 3175, 56, 15, 2388, 2836)
...
## 6. Evolución del número de fallecidos diarios por causa no violenta en el 2020 por sexo, edad, y nivel de instrucción
